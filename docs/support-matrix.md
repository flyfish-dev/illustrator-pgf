...
| CS–CC legacy deflate | `%AI12…23_CompressedData` + bounded zlib；支持 codec header 紧邻 marker 和前置 alternate private block | high | implemented；真实语料 oracle-pending |
| AI 2020+ zstd | `%AI24_ZStandard_Data`；支持 frame magic 紧邻 marker；Node 原生 zstd 能力探测；浏览器可注入本地 decoder | high | implemented envelope；浏览器 WASM/真实语料 oracle-pending |
...
| Lossless token/AST | 顺序、raw、span、注释、字符串、hex/ASCII85/binary、`%%BeginData`、gradient data mark、未知语法 | exact | byte-roundtrip tested |
| 画板 | BoundingBox、合成 `%AIArtboard`、部分 ArtboardArray 提取；共享画布 | partial | real modern metadata oracle-pending |
| 图层/组 | `Lb/Ln/LB`、`u/U`、compact/extended flags、状态和层级 | high | synthetic tested；version corpus pending |
| Path/CompoundPath | move/line/cubic、AI5 `L/C`、indirect paint、close、fill/stroke、evenodd/nonzero、`*u/*U` | high | synthetic tested |
| ClipGroup | clipping path 与结构化 clip group | high | synthetic tested |
| Transform | matrix composition；节点保存原矩阵 | high | synthetic tested |
| Gray/RGB/CMYK/Spot | 原色空间保留；AI5 `Xk/XK` custom color alternate；Canvas/SVG 对 CMYK/Spot 明确近似 | partial | ICC/Oracle pending |
...
| Opacity/blend/overprint | 基础状态操作符及 Illustrator `Xy` opacity；Canvas/SVG 可用子集 | partial | transparency oracle-pending |
...
| File Viewer adapter | auto/pdf/native 路由与 native session | partial | 本地主仓真实 AI/AIT、图层、移动端和浏览器 zstd 已验证；独立包发布与公开发布门禁 pending |
